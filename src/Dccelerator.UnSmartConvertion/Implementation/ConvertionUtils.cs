...
namespace Dccelerator.UnSmartConvertion
{
    ...
    public static class ConvertionUtils
    {
        ...
        static ConvertionRule GetRuleFor(Type sourceType, Type targetType, object value) {
            ...
            rule = _allRules.Select(x => {
                var r = x.Clone();
                r.PrepareConvertion(sourceType, targetType, x);
                return r;
        ...
        static readonly ConvertionRule[] _allRules = {
            ...
            new ConvertionRule {
                ...
                Convert = (source, target, value, thisRule) => {
                    try {
                        ...
                        Log.TraceEvent(TraceEventType.Error, $"{target} is an enum, but convertion from  {source} is failed.\n{e}");
                        return null;
                    }
                }
            },
            ...
            new ConvertionRule {
                ...
                Convert = (source, target, value, thisRule) => {
                    ...
                    Log.TraceEvent(TraceEventType.Warning, $"Can't parse Guid from {value}");
                    return null;
                }
            },
            ...
            new ConvertionRule {
                ...
                Convert = (source, target, value, thisRule) => {
                    ...
                    Log.TraceEvent(TraceEventType.Warning, $"Can't parse DateTime from string '{stringValue}'");
                    return null;
                }
            },
            ...
            new ConvertionRule {
                ...
                Convert = (source, target, value, thisRule) => {
                    try {
                        ...
                        Log.TraceEvent(TraceEventType.Error, $"{source} is an {TypeCache.ConvertibleType}, but changing it's type to {target} fails.\n{e}");
                        return null;
                    }
                }
            },
        ...
        public static object ConvertTo<T>(this T value,  Type targetType) {
            ...
            Log.TraceEvent(TraceEventType.Error, $"Can't find rule for converting {sourceType} into {targetType}");
            return null;
